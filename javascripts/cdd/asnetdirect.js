...
<div id="delaycar" onmouseleave="hideJanOptions()" style="border-radius: 10px; margin-top:110px; padding:5px; background-color:black; display: none;    margin-left: -212px;    position: fixed;">
...
<input type="button" value="Change list: ` + localStorage.getItem("listname") + `" id="resetMem" class="janlogin" onmousedown="resetMem()" style="background-color: darkgreen;color:#FFF;margin-top: 3px; font-size:small;" />
<input type="button" value="Add Price Fields" title="Add price fields, also use alt key" id="addprices" class="janlogin" onmousedown="refreshCode()" style="background-color: darkgreen;color:#FFF;margin-top: 3px; height:60px; font-size:small;" />
...
<span id="noteprice" style="font-family: sans-serif;    font-size: small;    color: cornsilk;">Black price: sent price</span>
...

	{
		...
		document.styleSheets[0].insertRule(".overlay { height: 100%; width: 0;    position: fixed;    z-index: 100000000;    top: 0;    left: 0;    background-color: rgb(0,0,0);    background-color: rgba(0,0,0, 0.9);    overflow-x: hidden;    transition: 0.5s;}", 0);
		document.styleSheets[0].insertRule(".overlay-content {    position: relative;    top: 25%;    width: 100%;    text-align: center;    margin-top: 30px;}", 0);

		document.styleSheets[0].insertRule(".overlay a {    padding: 8px;    text-decoration: none;    font-size: 36px;   color: red;  display: block;    transition: 0.3s;}", 0);

		document.styleSheets[0].insertRule(".overlay a:hover, .overlay a:focus {    color: grey }", 0);

		document.styleSheets[0].insertRule(".overlay .closebtn {    position: absolute;    top: 20px;    right: 45px;    font-size: 60px;}", 0);

		document.styleSheets[0].insertRule(".overlay a {font-size: 20px}", 0);
		document.styleSheets[0].insertRule(".overlay .closebtn {    font-size: 40px;    top: 15px;    right: 35px;  }", 1);

		document.styleSheets[0].insertRule(".mycheckbox{margin-right:3px; ", 0);

		document.styleSheets[0].insertRule("label{font-size:small; ", 0);

		document.styleSheets[0].insertRule(".newcar{ border: 4px solid yellowgreen !important;     border-radius: 10px;}", 0);
		document.styleSheets[0].insertRule(".visited{ border: 4px solid orange !important;    border-radius: 10px;}", 0);


		document.styleSheets[0].insertRule(".mytype { font-size: large; margin-top: 5px; font-weight: bold;}", 0);

		document.styleSheets[0].insertRule("div#error_view_area {    float: right;       background-color: aquamarine;}", 0);

		document.styleSheets[0].insertRule("#janoptions { width: 100%;    background-color: black;    color: aliceblue;    margin-top: 5px;    cursor: pointer; padding: 2px;    text-align: center;}", 1);

		document.styleSheets[0].insertRule("div#lever {    width: 100px;    height: 50px;    position: fixed;    background-color: red;    z-index: 20000;    opacity: 0.6;    border-radius: 5px;    margin-top: -20px;    color: white;    cursor: pointer; display:none;}", 0);

		document.styleSheets[0].insertRule(".credo{border-radius: 10px; float: left; position:fixed;    display: grid;   margin-top:10px; margin-left: -250px;    z-index: 1000000;    background-color: cadetblue; padding: 5px; width:230px; transition: margin-left 1s;}", 0);
		document.styleSheets[0].insertRule(".detprogbar { height: 26px;    width: 2%;    margin-left: 1%;    background-color:red;    float: left;    transition: background-color 0.25s;}", 0);
	...
	if (window.origin == "https://www.iauc.co.jp") {
		...
		} else if (pageUrl.href.match(/(thumbnail)|(preint)/g) != null) {
...
function getAlist(x) {
	...
	for (var i = 0; i < alist.length; i++) {
		if (alist[i].innerText.indexOf(x) >= 0) {
...
function changeSideBar() {
	...
	if (window.innerHeight < 740) {
		...
		$("#delaycar").css("display", "none");
		$("#delaycar").css("margin-left", "-212px");
...
function init() {
	...
	var lr = document.getElementsByTagName("table")[8].children[0].children;
	for (var i = 0; i < lr.length; i += 2) {
		...
		for (var k = 0; k < document.getElementsByTagName("table")[8].children[0].children[i].children.length; k++) {
			//console.log(k, "k");
			var drowx = document.createElement("div");
			drowx.setAttribute("class", "rowx");
			document.getElementsByTagName("table")[8].children[0].children[i].children[k].children[0].append(drowx);
		}
	...
	for (var j = 0; j < document.getElementsByClassName("rowx").length; j++) {
		$(".rowx").eq(j).append("<div class='detprogbar'></div>");
		//$(".rowx").eq(i).append("<button class='rowb' onclick='clickHandle(event)'>PHOTOS</button>"); 
		$(".rowx").eq(j).append("<input placeholder='price' class='rowp' onkeyUp='changepricecolor(event)' onkeypress='enterPressed(event)' onmouseup='windowFocused(event)'>");
		$(".rowx").eq(j).append("<input placeholder='comment' class='rowcom' onkeyUp='changepricecolor(event)' onkeypress='enterPressed(event)'>");

		var tlink = $(".rowp").eq(j).parent().parent().find("a")[0].href;
		
		

		$(".rowp").eq(j).val(checklocaldata(tlink));
...
function injectSayuri() {
	...
	document.body.appendChild(sayscript);
...
function enterPressed(event) {
	...
	if (x == 13) {
		...
		//$(".rowp").eq(0).parent().parent().children().eq(0).children().eq(2).children()[0].innerText;
		var url = $(event.target).parent().parent().find("a")[0].href;

		url = url.split("&");
		for (var i = 0; i < url.length; i++) {
			url[i] = url[i].split("=");

			if (url[i][0] == "holdingid") {
				holdid = url[i][1];
			} else if (url[i][0] == "auctionid") {
				aucid = url[i][1];
			}
		...
		var newrl = "https://www.iauc.co.jp/detail/?vehicleId=55-" + event.target.parentElement.getElementsByClassName("rowp")[0].lot + "-" + holdid;
		event.target.parentElement.getElementsByClassName("rowp")[0].lot = event.target.parentElement.getElementsByClassName("rowp")[0].lot + "//" + holdid; 

		//event.target.parentElement.getElementsByClassName("rowp")[0].grade = translateGrade(event.target.parentElement.getElementsByClassName("rowp")[0].grade);
		if ($("#jans_country").val() != "") {
			...
			if (selcon > 0) {
				...
				//chassis = chassis.substr(chassis.lastIndexOf(" "));
				var ssd = [];
				...
				localStorage.setItem(event.target.parentElement.parentElement.getElementsByTagName("a")[0].href, p);
				...
				if (cbl.indexOf("ask") >= 0) {
					...
					if (sendOK) {
						...
						if (SendDataToDB == "true") {
							sendtoDatabase(event.target, event.target.parentElement.getElementsByClassName("rowp")[0], pb, cb, newrl);
							/*
							document.getElementById("auction_to_send").innerHTML = "";
							document.getElementById("lot_to_send").innerHTML = "";
...
function changepricecolor(event) {
	...
	var newrl = event.target.parentElement.parentElement.getElementsByTagName("a")[0].href; //main link to fetch more data
...
function enterPressedInd(event) {
	...
	if (x == 13) {
		...
		var chascode = getChassisInd().replace(/\s/g, " ");
		...
		localStorage.setItem(priceLink, p);
...
function windowFocused(event) {
	...
	var et = event.target;
	var photo;
	var newrl = $(event.target).parent().parent().find("a")[0].href;
	
	
	et.carname =$(et).parent().parent().find("span")[0].innerText;
	...
	function showDetails(d) {
		for (var i = 0; i < $(d).find(".predt_css").length - 1; i += 2) {
			//console.log($(d).find(".predt_css[bgcolor='#CCCCFF']").filter(":contains('Lot')").siblings()[0].innerText);
			if ($(d).find(".predt_css")[i].innerText.indexOf("Lot") >= 0) {
				et.lot = $(d).find(".predt_css").eq(i + 1).text().trim();
			} else if ($(d).find(".predt_css")[i].innerText.indexOf("Condition") >= 0) {
				et.rate = $(d).find(".predt_css").eq(i + 1).text().trim();
			} else if ($(d).find(".predt_css")[i].innerText.indexOf("Km") >= 0) {
				et.mileage = $(d).find(".predt_css").eq(i + 1).text().trim();
			} else if ($(d).find(".predt_css")[i].innerText.indexOf("Grade") >= 0) {
				et.grade = $(d).find(".predt_css").eq(i + 1).text().trim();
			} else if ($(d).find(".predt_css")[i].innerText.indexOf("Exterior") >= 0) {
				et.ext = $(d).find(".predt_css").eq(i + 1).text().trim();
			} else if ($(d).find(".predt_css")[i].innerText.indexOf("Interior") >= 0) {
				et.inter = $(d).find(".predt_css").eq(i + 1).text().trim();
			} else if ($(d).find(".predt_css")[i].innerText.indexOf("Year") >= 0) {
				et.year = $(d).find(".predt_css").eq(i + 1).text().trim();
			} else if ($(d).find(".predt_css")[i].innerText.indexOf("cc") >= 0) {
				et.cc = $(d).find(".predt_css").eq(i + 1).text().trim();
			} else if ($(d).find(".predt_css")[i].innerText.indexOf("Color") >= 0) {
				et.color = $(d).find(".predt_css").eq(i + 1).text().trim();
			} else if ($(d).find(".predt_css")[i].innerText.indexOf("A/C") >= 0) {
				et.ac = $(d).find(".predt_css").eq(i + 1).text().trim();
			} else if ($(d).find(".predt_css")[i].innerText.indexOf("Fuel") >= 0) {
				et.fuel = $(d).find(".predt_css").eq(i + 1).text().trim();
			} else if ($(d).find(".predt_css")[i].innerText.indexOf("Seats") >= 0) {
				et.seats = $(d).find(".predt_css").eq(i + 1).text().trim();
			} else if ($(d).find(".predt_css")[i].innerText.indexOf("Transmission") >= 0) {
				et.trans = $(d).find(".predt_css").eq(i + 1).text().trim();
			} else if ($(d).find(".predt_css")[i].innerText.indexOf("Registration") >= 0) {
				et.reg = $(d).find(".predt_css").eq(i + 1).text().trim();
			}  else if ($(d).find(".predt_css")[i].innerText.indexOf("Auction House") >= 0) {
				et.aucname = getAucname($(d).find(".predt_css").eq(i + 1).text().trim());
			} else if ($(d).find(".predt_css")[i].innerText.indexOf("Starting") >= 0) {
				et.starting = $(d).find(".predt_css").eq(i + 1).text().trim();
			}
	...
	if (!event.ctrlKey) {
		...
		function openIt() {
			...
			try {
				myWindow = window.open(newrl, "Images", "width=1010,height=970,titlebar=yes");
...
function loadIt(et) {
	...
	var newrl = et.parentElement.parentElement.getElementsByTagName("a")[0].href; //main link to fetch more data
		...
		$.get(newrl, function (data) {
			...
			.done(function () {
				...
				if (myWindow != undefined || myWindow != null) {
					...
					//myWindow.document.body.innerHTML = $(photo).find("table")[16].innerHTML;

					var pics = $(photo).find("form").eq(1).find("img");

					console.log(pics);
					//myWindow.document.body.append(pics);
...
function showDetails(d) {
	for (var i = 0; i < $(d).find(".predt_css").length - 1; i += 2) {
		if ($(".predt_css").eq(0).attr("bgcolor") == "#CCCCFF") {
			//blue found
			console.log($(".predt_css").eq(k + 1).text().trim());
		}
...
function getChassis(pt) {
	...
	try {
		chassis = pt.find("#detail-name")[0].innerText.trim();
		return chassis;
	} catch (e) {
		return "";
	}
...
function injectImages(dp) {
	imgArray = [];


	//$(dp).find(".predt_css[bgcolor='#CCCCFF']").filter(":contains('Lot')").siblings()[0];
	/*
	for (var i = 0; i < dp.find("form").eq(1).find("img").length; i++) {
		if (dp.find("form").eq(1).find("img")[i].src.length > 40) {
			console.log(photo.find("form").eq(1).find("img")[i].src + " image source");
			imgArray.push(dp.find("form").eq(1).find("img")[i]);
		}
		
	}
	*/

	//console.log(photo.find("form"));


	detailRows = [];
	var r = dp.find(".predt_css");
	...
	var divNode = myWindow.document.createElement("style");
	divNode.innerHTML = `body{background-color:white;} #image{width:620;} #leftarrow {	position:absolute; top: 500px; left: 0px; cursor: pointer;}	#rightarrow {position:absolute; top: 500px; left: 520px; cursor: pointer;} h1{background:olive;} #signin-button{margin-bottom:100px} #signout-button{margin-bottom:100px} .thumb{cursor:pointer; width:80px; height:60px; display:inline-grid; margin:2px;} #cont{height:145px; width:610px; overflow-y: scroll; white-space:wrap; overflow-x:scroll;border-style: double;    border-color: dodgerblue;    background-color: orange;} 
...
.col-md-4.col-nomargin.like-tbl-outer {
    width: 33%;
	float:left;
}
h4.col-md-4.col-xs-3.col-height.col-middle {
    padding: 0px;
    margin: 0px;
...
p.col-md-8.col-xs-9.col-height.col-middle {
    margin: 2px;
}
h4#col-left {
    height: 23px;
...
p.col-md-4.col-xs-9.col-height.col-middle {
    margin: 1px;
    padding: 1px;
}
.like-tbl-outer {
...
.col-xs-3.col-md-3.col-nomargin {
    height: 40px;
}
	...
	var cc = getChassis(dp).replace(/\s+/g, " ");

	//
	if (cc.indexOf("oyota") > 0 || cc.indexOf("OYOTA") > 0) {

		var tcc = cc.substr(cc.lastIndexOf(" ") + 1);
		tcc = tcc.substr(0, cc.lastIndexOf("-"));


		if (tcc.match(/([\d]+)([a-zA-Z]{1})/g)) {
			tcc = tcc.substr(0, tcc.length - 1);
			//console.log("last is letter", tcc);
		}
		//console.log(tcc);
	}

	cc = cc.substr(cc.lastIndexOf(" ") + 1, cc.length - 1);
...
function janlogin() {
	...
	/*
		$.ajax({
    //url: resource_url+"www/test/logincheck.php",
...
function sendtoDatabase(et, rowb, pb, cb, durl) {

	//url: resource_url+'www/test/managedata.php',
	console.log(rowb.aucname, durl);
			...
			$.ajax({
				...
				data: {
					...
					ext: rowb.ext,
					inter: rowb.inter,
					start_price: rowb.starting,
					start_time: rowb.starttime2,
				...
				beforeSend: function () {
					...
					console.log("sending");
					//document.getElementById("lot_to_send").innerText = rowb.lot.substr(rowb.lot.indexOf("//") + 2);
					//document.getElementById("auction_to_send").innerText = pb;
					//document.getElementById("send_status").innerText = "ESC to cancel";
					localStorage.setItem("lastVehicle", {
						"lot": rowb.lot.substr(rowb.lot.indexOf("//") + 2),
						"price": pb
					});
...
function loadJquery() {
	...
	era.id = "error_view_area";
	document.body.insertBefore(era, document.body.firstChild);
