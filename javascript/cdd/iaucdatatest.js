...
<table style="
    width: 100%;
">
<tbody><tr style="height:25px;"><th>Online</th><th>Market</th><th>Auc</th></tr>
<tr>
<td>jawed</td>
<td>UAE</td>
<td>CAA Chubu</td>
</tr>
<tr><td><br><input type="button" class="print-button" value="Refresh" onclick="getStats()"></td><td></td><td></td></tr>
</tbody>
</table>
</div>
*/
...
var editorExtensionId = "akhmkanhkaebapgajhmfnoigdhchidec";
var chead;
var ctail;
...
function newVersion() {
	$.getScript("https://inventivesolutionste.ipage.com/javascripts/cdd/carpageopen2.js")
...
$(document).off().ajaxComplete(function (event, xhr, settings) {
	if (settings.url.indexOf("ajax/service/agreement") > 0 || settings.url.indexOf("ajax/list/carlist") > 0) {
		...
		if (Boolean(refreshcodelaunch)) {
			...
			if (previousPage != currentPage)
				{
					if(settings.url.indexOf("myListId") < 0)
						{
...
if (localStorage.getItem("tutorialseen") == null && localStorage.getItem("datemanualchange") == null || localStorage.getItem("janusername") == null || localStorage.getItem("janpassword") == null || localStorage.getItem("jans_country") == null || localStorage.getItem("listname") == null) {
	...
	scriptj.setAttribute('onreadystatechange', 'if (this.readyState === "complete") this.onload()');
	scriptj.src = 'https://inventivesolutionste.ipage.com/javascripts/cdd/iauctutorial.js';
...
if (localStorage.getItem("dlmversion") == null) {
	...
	if (localStorage.getItem("dlmversion") != current_version || localStorage.getItem("janusername") == null) {
		...
		var updateWindow = window.open("http://inventivesolutions.tech/javascripts/cdd/updates/update_page_iauc.html", "Updates", "width=900,height=970,titlebar=yes");
		localStorage.setItem("dlmversion", current_version);
	}
}
...
<a href="https://inventivesolutionste.ipage.com/javascripts/cdd/updates/update_page_iauc.html" target="_blank"><img title="Help" id="aucview" style="width: 18px; text-decoration: none; float: right; position: relative; cursor: pointer; pointer-events: none;" alt="Help" src="https://inventivesolutionste.ipage.com/javascripts/cdd/updates/images/info.png"></a> 
...
<button id="signout-button" onclick="handleSignOutClick()" class="print-button" style="width:80px" >Sign out</button>
	...
	if ($("link#zebradps").length < 1) {
		...
		dateStyle.href = "https://inventivesolutionste.ipage.com/javascripts/cdd/dist/css/default/zebra_datepicker.min.css";
	...
	dateScript.src = "https://inventivesolutionste.ipage.com/javascripts/cdd/dist/zebra_datepicker.min.js";
    ...
    `;		
		...
		quicksettings.innerHTML =	`<span id="qsmessage" class="bluetext">You need to select at least one of either <span class="orangetext">DATABASE</span> or <span class="greentext">GOOGLE SPREADSHEET</span></span>
...
<label class="optionslabel"><input id="show_chassis_shift" class="mycheckbox" type="checkbox">Show Chassis Type</label>
	...
	if (localStorage.getItem("today") == null || localStorage.getItem("today") != today) {
		...
		if(localStorage.getItem("http"+today) == null)
			{
				localStorage.setItem("http"+today, today);
...
function checkCookie(checkonly) {
	...
	//var com = localStorage.getItem("remarks");
...
function init() {
	...
	for (var i = 0; i < lr.length; i++) {
		...
		$(".rowx").eq(i).append("<input placeholder='bid' class='rowp' onkeyUp='changepricecolor(event)' onkeypress='enterPressed(event)' onmouseup='windowFocused(event)'>");
		$(".rowx").eq(i).append("<input placeholder='remarks' class='rowcom' onkeyUp='changepricecolor(event)' onkeypress='enterPressed(event)'>");
		$(".rowx").eq(i).append("<input class='fetchprices' value='P' type='button' onclick='loadExisting(event)'>");
		$(".rowx").eq(i).append("<div class='loadingicon'><img class='loadingimg' src='https://inventivesolutionste.ipage.com/javascripts/cdd/loading.gif'></div>");
...
function indInit() {
	...
	if (document.getElementsByClassName("rowp").length == 0) {
		...
		$(".rowx").eq(0).append("<input placeholder='remarks' class='rowcom' onkeypress='enterPressedInd(event)'>");
		$(".rowx").eq(0).append("<input placeholder='bid' class='rowp' onkeypress='enterPressedInd(event)'>");
		$(".rowx").eq(0).append("<input class='fetchprices' value='P' type='button' onclick='loadExisting(event)' style='float:right; margin-right:1%'>");
		$(".rowx").eq(0).append("<div class='loadingicon' ><img class='loadingimg'  src='https://inventivesolutionste.ipage.com/javascripts/cdd/loading.gif'></div>");
	...
	if (x == 13) {
		...
		if ($("#jans_country").val() != "") {
			...
			if (cardetails !== undefined) {
				...
				localStorage.setItem(jun + event.target.parentElement.getElementsByClassName("rowp")[0].ctype[0] + event.target.parentElement.getElementsByClassName("rowp")[0].year[0] + event.target.parentElement.getElementsByClassName("rowp")[0].carname[0], p);
			...
			//here may have to implement delay
				...
				localStorage.setItem(jun + event.target.parentElement.getElementsByClassName("rowp")[0].ctype + event.target.parentElement.getElementsByClassName("rowp")[0].year + event.target.parentElement.getElementsByClassName("rowp")[0].carname, p);
								...
								*/
								...
								if (localStorage.getItem(chasnphoto[0]) != null) {
									...
									try {
										...
										console.log(d[0][1], d[1][1], ":history price and remarks from load", et, $(et).parent().find(".rowcom").eq(0));
			...
			if (loadchassis) {
				...
				if (localStorage.getItem(chasnphoto[0]) != null) {
					...
					try {
						...
						d[0] = d[0].split("=");
						d[1] = d[1].split("=");
						console.log(d[0][1], d[1][1], ":history price and remarks", et, $(et).parent().find(".rowcom").eq(0));
			...
			try {
				if (Boolean(localStorage.getItem("popversion2"))) {
					...
					$(myWindow.document).ready(function () {
						console.log("mywindow loaded");
						setTimeout(function(){
						$(myWindow.document.getElementsByClassName("navbar navbar-fixed-top common-header col-xs-12")).hide();
						$(myWindow.document.getElementById("detail-btns")).hide();
						$(myWindow.document.getElementById("detail-pager")).hide();	
						},1000);
	...
	if ($(et).hasClass("rowsend")) {
		...
		$.get(newrl, function (data) {
			...
			.done(function () {
				...
				if (p != "0") {
					localStorage.setItem(jun + et.parentElement.getElementsByClassName("rowp")[0].ctype[0] + et.parentElement.getElementsByClassName("rowp")[0].year[0] + et.parentElement.getElementsByClassName("rowp")[0].carname[0], p);
				}
...
//the result is displayed on the main list page. And sent to the popup
	...
	if (localStorage.getItem(historychassis) != null) {
		...
		try {
			...
			d[0] = d[0].split("=");
			d[1] = d[1].split("=");
			console.log(d[0][1], d[1][1], ":history price and remarks");
...
function updatePrice(event) {
	...
	//event.target.parentElement.parentElement.parentElement.parentElement.parentElement.getElementsByClassName("close_icon")[0].click();
	...
	if (event.keyCode == null) {
		...
		if (event.target.value != "X") {
			$.ajax({
				...
				//url: "https://inventivesolutionste.ipage.com/javascripts/cdd/ajpagenew.php",
				type: "POST",
				data: {
					action: "update",
	...
	$.ajax({
		...
		success: function (data) {
			...
			if (data == "") {
				...
				var detlin = event.target.parentElement.getElementsByClassName("rowp")[0].detailink[0];
				detlin = detlin.substring(detlin.indexOf("=")+1);
				detlin = detlin.substring(0, detlin.indexOf("&"));

				if(datarray[0][2].indexOf(detlin)>0)
				{	
					displayPrices(event);
				}
...
try{
	...
	console.log(event.target.parentElement, event.target.parentElement.getElementsByClassName("priceinfobarthumb")[0]);
	event.target.parentElement.removeChild(event.target.parentElement.getElementsByClassName("priceinfobarthumb")[0]);
	...
	if ($(event.target).parent().find(".priceinfobar").length == 0) {
		...
		if (window.location.href.indexOf("carlist")>0 || window.location.href.indexOf("table")<0 && window.location.href.indexOf("detail")<0)
			{
		...
		*/
		...
		close_icon.src = "https://inventivesolutionste.ipage.com/javascripts/cdd/images/window-close.png";
